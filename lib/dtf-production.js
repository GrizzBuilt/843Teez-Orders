...
function calculateDtfLocationCost({
  quantity,
  locationCount,
  costPerLocationCents,
}) {
  const normalizedQuantity = Math.max(0, Math.floor(Number(quantity) || 0));
  const normalizedLocationCount = Math.max(
    1,
    Math.floor(Number(locationCount) || 0)
  );
  const normalizedCostPerLocationCents = Math.max(
    0,
    Math.round(Number(costPerLocationCents) || 0)
  );
  const costPerShirtCents =
    normalizedCostPerLocationCents * normalizedLocationCount;

  return {
    location_count: normalizedLocationCount,
    cost_per_location_cents: normalizedCostPerLocationCents,
